...
export function commentAddedTemplate(p: {
  ticketNumber: string;
  ticketTitle: string;
  commentBody: string;
  authorName: string;
  ...
  return {
    subject: `[${p.ticketNumber}] New comment from ${p.authorName}`,
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto">
        <h2 style="color:#1a1a2e">New comment on your ticket</h2>
        <p>Hi ${esc(p.recipientName)},</p>
        <p><strong>${esc(p.authorName)}</strong> left a comment on ticket <strong>${esc(p.ticketNumber)}</strong>.</p>
